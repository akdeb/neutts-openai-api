...
export const ModelsPage = () => {
  ...
  const save = async () => {
    try {
      setSaving(true);
      setError(null);
      const data = await api.setModels({ model_repo: modelRepo, model_file: modelFile });
      setCurrent(data);
    } catch (e: any) {
    ...
    <div>
      ...
      <div className="retro-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <div className="font-bold uppercase text-xs mb-1">Model file</div>
            <input className="retro-input w-full" value={modelFile} onChange={(e) => setModelFile(e.target.value)} />
            <div className="text-[10px] font-mono text-gray-500 mt-1">Example: llama-3.2-3b-instruct-q4_k_m.gguf</div>
          </div>
        </div>
